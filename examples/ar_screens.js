...
export function createNewBrowserScreen(
  ...
  const browserWindow = enhancedCreateScreen(
    position,
    size,
    title,
    videoTexture
  ...
  const topBar = browserWindow.children.find(
...
export function createYouTubeScreen(position = new THREE.Vector3(0, 0, -1.5)) {
  ...
  // Create a real YouTube texture using the video ID
...
export function createDuckDuckGoScreen(
  position = new THREE.Vector3(0, 0, -1.5)
) {
  // Screen dimensions
  const screenWidth = 1.0;
  ...
  const iframeTexture = createIframeTexture(
  ...
  // Create the screen container
  const duckduckgoScreen = enhancedCreateScreen(
    position,
    size,
    title,
    iframeTexture
  ...
  // Update drag handle reference
  const topBar = duckduckgoScreen.children.find(
    (child) => child.userData && child.userData.type === "dragHandle"
  );
  ...
  console.log(
    "Created DuckDuckGo screen with ID:",
    duckduckgoScreen.userData.id
  );
...
export function createGoogleMapsScreen(
  position = new THREE.Vector3(0, 0, -1.5)
) {
  // Screen dimensions
  const screenWidth = 1.0;
...
export function createElectronAppScreen(
  position = new THREE.Vector3(0, 0, -1.5)
) {
  // Screen dimensions
  const screenWidth = 1.0;
  ...
  console.log(
    "Created Electron App screen with ID:",
    electronScreen.userData.id
  );
...
function addControlButton(screen, type, x, y, size) {
  // Create button background with improved appearance
  const buttonGeometry = new THREE.CircleGeometry(size, 32); // More segments for smoother circle
  ...
  button.position.set(x, y, 0.01); // Slightly in front of screen
  button.renderOrder = 20; // Higher render order to ensure it renders on top

  // Set up user data for interaction
  button.userData = {
    type: "button",
  ...
  createControlIcon(type)
    .then((iconTexture) => {
      iconMaterial.map = iconTexture;
      iconMaterial.needsUpdate = true;
    })
    .catch((error) => {
      console.error("Error loading control icon:", error);
    });
...
function createFallbackTexture(screenNumber) {
  ...
  ctx.fillText(
    "Tap to interact with the player",
    canvas.width / 2,
    canvas.height / 2 + 140
  );
...
export function selectScreen(screen) {
  ...
  if (selectedScreen) {
    ...
    const borderMesh = selectedScreen.children.find(
      (child) =>
        child.geometry &&
  ...
  const borderMesh = screen.children.find(
    (child) =>
      child.geometry &&
      child.geometry.type === "PlaneGeometry" &&
      Math.abs(child.position.z - -0.001) < 0.0001
...
function animateScreenScale(screen, targetScale, duration, bounce = false) {
  const originalScale =
    screen.userData.originalScale || new THREE.Vector3(1, 1, 1);
...
// Create a YouTube video texture from a video ID
function createYouTubeVideoTexture(videoId) {
  // Create a canvas that will act as our texture
  const canvas = document.createElement("canvas");
  canvas.width = 1024;
  canvas.height = 768;
  const ctx = canvas.getContext("2d");
